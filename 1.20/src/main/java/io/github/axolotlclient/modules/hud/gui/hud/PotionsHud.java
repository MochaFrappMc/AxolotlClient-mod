...
public class PotionsHud extends TextHudEntry implements DynamicallyPositionable {
	...
	private void renderEffects(GuiGraphics graphics, List<StatusEffectInstance> effects, float delta) {
		...
		for (int i = 0; i < effects.size(); i++) {
			...
			if (direction.isXAxis()) {
				renderPotion(graphics, effect, x + lastPos + 1, y + 1, delta);
				...
				renderPotion(graphics, effect, x + 1, y + 1 + lastPos, delta);
				lastPos += 20;
			}
		}
	}
	...
	private void renderPotion(GuiGraphics graphics, StatusEffectInstance effect, int x, int y, float delta) {
		...
		if (!iconsOnly.get()) {
			if (showEffectName.get()) {
				...
				Text duration = StatusEffectUtil.durationToString(effect, 1, delta);
				graphics.drawText(client.textRenderer, duration, x + 19, y + 1 + 10, 8355711, shadow.get());
			} else {
				graphics.drawText(client.textRenderer, StatusEffectUtil.durationToString(effect, 1, delta), x + 19, y + 5,
